# ADR Index

Architecture Decision Records (ADRs) are the primary decision log for shared Friction architecture and governance.

## ADRs

- [ADR-0001 Repo Boundary Strategy](./ADR-0001-repo-boundary-strategy.md)
- [ADR-0002 Event-Driven Architecture with RxJava 3](./ADR-0002-event-driven-architecture-rxjava3.md)
- [ADR-0003 EO Naming Constraints](./ADR-0003-eo-naming-constraints.md)
- [ADR-0004 Read Model Interfaces as Pluggable Contracts](./ADR-0004-read-model-pluggable-interfaces.md)
- [ADR-0005 Label-Driven Versioning Policy](./ADR-0005-label-driven-versioning.md)
- [ADR-0006 Observation Clustering Strategy](./ADR-0006-observation-clustering-strategy.md)
- [ADR-0007 Metrics Semantics and Edge-Case Policy](./ADR-0007-metrics-semantics-and-edge-cases.md)
- [ADR-0008 Friction Descriptor Generation Policy](./ADR-0008-descriptor-generation-policy.md)
